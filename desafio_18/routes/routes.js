...
router.get("/logerror", (req, res) =>
  res.render("error", {
    text: "Usuario o contraseña incorrectos.",
    type: "logerror",
  })
)
router.get("/regerror", (req, res) =>
  res.render("error", {
...
router.get("/products", isAuth, (req, res) =>
  db
    .getAll()
    .then((data) => res.render("products", { user: req.user, data: data }))
)
...
router.post("/buy", isAuth, async (req, res) => {
  cartDB.findCartByID(req.user.cartID).then((response) => {
    ...
    response.products.forEach(
      (element) =>
        (emailContent += `<p>Item: ${element.itemName} | Precio: ${element.itemPrice}</p>`)
    )
